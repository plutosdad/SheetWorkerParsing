var ExExp = ExExp || (function () {
	...
	());
/* SWUtils is only for interaction with ExExp */
var SWUtils = SWUtils || (function () {
		...
		var validNumericStr = function (preeval) {
			...
			evaluateExpression = function (exprStr, callback) {
				...
				var bmatches1,bmatches2,pmatches1,pmatches2,smatches1,smatches2;
			...
			/* called only by evaluateAndSetNumber */
			 ...
			 *
			 */
			evaluateAndSetNumber = function (readField, writeField, defaultVal, callback, silently) {
				var done = function (a, b, c) {
						if (typeof callback === "function") {
							callback(a, b, c);
						}
					},
					...
					try {
						if (typeof values[readField] === "undefined") {
							...
							evaluateExpression(values[readField], function (value2) {
								try {
									if (value2 === null || value2===undefined || isNaN(value2)) {value2=trueDefault;}
									if (isNaN(currVal) || currVal !== value2) {
										setter[writeField] = value2;
										setAttrs(setter, params, function () {
											done(value2, currVal, true);
										});
									} else {
										done(currVal, currVal, false);
									}
								} catch (err2) {
									TAS.error("evaluateAndSetNumber after evaluateExpression ", err2);
									done(currVal, currVal, false);
								}
							});
						}
					} catch (err) {
						TAS.error("evaluateAndSetNumber", err);
						done(value, currVal, false);
					}
				});
			},
			...
			copyStaticNumberFieldToRepeating = function (repeatingSection, copyFromField, fieldToUpdatePartialName) {
				if (!fieldToUpdatePartialName) {
					fieldToUpdatePartialName = copyFromField + "-copy";
				}
			 ...
			 *
			 * @readField {string} = the dropdown field
			...
			handleDropdown = function (readField, writeFields, synchrousFindAttributeFunc, callback, silently) {
				var params = {},
					done = function (newval, currval, changed) {
						if (typeof callback === "function") {
			 ...
			 * @fields {Array}= array of field names, first element (fields[0]) MUST be the total field ,
			 * @bonus {number} = a number that is added
			 * @penalties {Array} = array of fieldnames to be subtracted from the total
			 * @totalIsFloat (boolean) = true if do not round total to int.
			 * @callback {function} 
			 */
			getRowTotal = function (fields, bonus, penalties, totalIsFloat, callback, errorCallback) {
				...
				try {
					...
					TAS.error("getRowTotal catastrophic error: ",err2);
				...
				getAttrs(readFields, function (v) {
					...
					try {
						...
						TAS.error("PFUtilsAsync.getRowTotal", err);
					} finally {
						callback(newValue, currValue, v);
					}
				});
			},
			/* updateRowTotal
